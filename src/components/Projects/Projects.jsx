...
import { projects, projectFiles } from '../../../constants/index.js';
...
const Projects = () => {
    // ---------- editor state ----------
    const [activeTab, setActiveTab] = useState('bilkent_navigator.json');
    ...
    // scope refs
    const sectionRef = useRef(null); // whole Projects section (positioning context)
    ...
    // marquee text
    const scrollingTextContent = useMemo(() => Array(20).fill('Projects ').join(' '), []);

    // files (raw + highlighted)
    const files = useMemo(() => {
        return projectFiles.map((file, idx) => {
            const { technologies, logo, ...rest } = projects[idx] || {};
            // For preview: include technologies and logo in raw only if defined
            const raw = {
                ...rest,
                ...(technologies !== undefined ? { technologies } : {}),
                ...(logo !== undefined ? { logo } : {})
            };
            // For IDE tab: exclude technologies and logo from content
    ...
    const [winPos, setWinPos] = useLocalStorage('proj_preview_pos', { x: 24, y: 24 });
    const [winSize, setWinSize] = useLocalStorage('proj_preview_size', { w: 800, h: 480 });

    // First mount: set initial position/size to center in current viewport (like handleMaximize)
    useLayoutEffect(() => {
        const hasSavedPos = !!localStorage.getItem('proj_preview_pos');
        const hasSavedSize = !!localStorage.getItem('proj_preview_size');
